...
class ReplicaLinkerThread(name: String,
  ...
  override def processPartitionData(
    ...
    if (logTrace)
      trace("Leader has replica log end offset %d for partition %s. Received %d bytes of messages and leader hw %d"
        .format(log.logEndOffset, topicPartition, records.sizeInBytes, partitionData.highWatermark))

    // WHEN WE WRITE, WE BEHAVE AS A FOLLOWER: we don't want to assign offsets, etc...
    // MAYBE AN ALTERNATE METHOD IN PARTITION WILL BE REQUIRED HERE!
    val logAppendInfo = partition.appendRecordsToFollowerOrFutureReplica(records, isFuture = false, partitionLeaderEpoch)

    if (logTrace)
      trace("Leader has replica log end offset %d after appending %d bytes of messages for partition %s"
        .format(log.logEndOffset, records.sizeInBytes, topicPartition))
    val leaderLogStartOffset = partitionData.logStartOffset

    // TODO we need to keep its segment base offset and physical position!!!!
    // For Followers, these values will be computed upon becoming leader or handling a preferred read replica fetch.
  ...
  private def completeDelayedFetchRequests(): Unit = {
  	// TODO: trigger something on the RelicaMgr??
