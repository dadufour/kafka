...
/**
 * Facilitates fetches from an external (other cluster) replica leader.
 *
 ...
 */
class ExternalLeaderEndPoint(logPrefix: String,
  ...
  private def fetchOffset(topicPartition: TopicPartition, currentLeaderEpoch: Int, timestamp: Long): OffsetAndEpoch = {
    ...
    val requestBuilder = ListOffsetsRequest.Builder.forConsumer(true, IsolationLevel.READ_UNCOMMITTED, true, true, true)
  ...
  override def fetchEpochEndOffsets(partitions: java.util.Map[TopicPartition, OffsetForLeaderEpochRequestData.OffsetForLeaderPartition]): java.util.Map[TopicPartition, EpochEndOffset] = {
    ...
    val epochRequest = OffsetsForLeaderEpochRequest.Builder.forConsumer(topics)
...
warn("FETCHDATA=" + fetchData.toString)
      ...
      val requestBuilder = FetchRequest.Builder
        .forConsumer(version, maxWait, minBytes, fetchData.toSend)
  ...
  override def toString: String = s"ExternalLeaderEndPoint(blockingSender=$blockingSender)"
}
